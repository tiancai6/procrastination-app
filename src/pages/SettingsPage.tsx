...
const SettingsPage: React.FC = () => {
  ...
  const handleExportData = async () => {
    try {
      ...
      Alert.alert(
        '导出备份',
        `已生成完整备份（含 ${recordCount} 条专注记录，以及规划/提醒、习惯打卡、记账与分类、三餐、AI 对话、随手记、头像图片和首页背景图）\n\n文件大小：${sizeText}\n\n${fileName}\n\n可通过微信"文件传输助手"发送到电脑，或"存储到文件"保存到 iCloud。${warning}`,
      );
    } catch (error) {
          ...
          onPress: async () => {
            try {
              ...
              if (result) {
                ...
                Alert.alert(
                  '导入成功',
                  `已恢复 ${result.recordCount} 条专注记录，以及全部规划/提醒、习惯打卡、记账与分类、三餐、AI 对话、随手记数据。\n\n请切换到其他页面查看最新数据。`,
                );
              }
            } catch (error) {
  ...
  const openAIModal = async () => {
    ...
    const v = await getVisionModel();
    setApiKeyInput(k || '');
    setAiModelInput(m || 'glm-4-flash');
    setVisionModelInput(v || 'glm-4v-flash');
    setAiEnabled(!!k);
    setShowAIModal(true);
  };
  ...
  const saveAISettings = async () => {
    ...
    await setVisionModel(visionModelInput.trim() || 'glm-4v-flash');
    setAiEnabled(!!key);
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.version}>
        <Text style={styles.versionText}>日迹 v1.0.0</Text>
      </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.aboutContent}>
              <Text style={styles.aboutText}>日迹</Text>
              ...
              <Text style={styles.aboutFeature}>• 记录每次专注时长与分类</Text>
              <Text style={styles.aboutFeature}>• 统计分析专注习惯</Text>
              <Text style={styles.aboutFeature}>• 任务规划提升执行力</Text>
              <Text style={styles.aboutFeature}>• 快速启动即时计时</Text>
              <View style={styles.divider} />
